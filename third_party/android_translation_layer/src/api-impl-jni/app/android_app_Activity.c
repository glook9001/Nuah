...
static void activity_focus(JNIEnv *env, jobject activity)
{
	...
	if ((*env)->ExceptionCheck(env))
		(*env)->ExceptionDescribe(env);
	/* AndroidX media-picker registration can race the synthetic desktop
	 * lifecycle. Do not leave this recoverable callback exception pending;
	 * ART treats the next JNI lookup with a pending exception as fatal. */
	if ((*env)->ExceptionCheck(env)) {
...
{
	/* A lifecycle observer may throw while AndroidX is synchronously notified
	 * during the synthetic desktop transition. Never carry that exception into
	 * the JNI field/method lookups below; ART treats that as a VM-fatal misuse. */
	if ((*env)->ExceptionCheck(env))
		(*env)->ExceptionClear(env);
...
JNIEXPORT void JNICALL Java_android_app_Activity_nativeStartActivity(JNIEnv *env, jclass class, jobject activity)
{
	...
	/* Lifecycle observers from the Roblox APK may report a registration
	 * failure during onStart. It is non-fatal for the desktop runtime, but a
	 * pending Java exception makes the next JNI lookup abort ART. */
	if ((*env)->ExceptionCheck(env)) {
		(*env)->ExceptionDescribe(env);
		(*env)->ExceptionClear(env);
	}
