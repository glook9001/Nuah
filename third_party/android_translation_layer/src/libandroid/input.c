...
static gboolean on_event(GtkEventControllerLegacy *self, GdkEvent *event, int input_queue_pipe_fd)
{
	...
	switch (gdk_event_get_event_type(event)) {
		...
		case GDK_TOUCH_UPDATE:
			...
			/* Never let a high-rate pointer stream block GTK's UI thread.  The
			 * Android input consumer can temporarily fall behind while rendering
			 * or loading a place; a blocking pipe write would then freeze the
			 * entire ATL window (including keyboard and menu navigation).  Events
			 * are complete, small records, so PIPE_BUF makes each accepted write
			 * atomic.  Dropping a motion event when the queue is full is safe; the
			 * next motion/click will re-synchronise the pointer state. */
			ssize_t written = write(input_queue_pipe_fd, &ainput_event,
			                        sizeof(struct AInputEvent));
			if (written < 0 && errno != EAGAIN && errno != EWOULDBLOCK && errno != EINTR)
				g_debug("ATL input queue write failed: %s", g_strerror(errno));
			break;
		default:
			return false;
			break;
...
void AInputQueue_attachLooper(struct input_queue *queue, struct ALooper *looper, int ident, Looper_callbackFunc callback, void *data)
{
	...
	/* The producer runs in GTK's event thread.  It must not block if the
	 * Android looper is busy, otherwise pointer movement freezes the UI. */
	fcntl(input_queue_pipe[1], F_SETFL, O_NONBLOCK);
