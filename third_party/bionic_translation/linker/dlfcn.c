...
static pthread_mutex_t apkenv_dl_lock;
static pthread_once_t apkenv_dl_lock_once = PTHREAD_ONCE_INIT;

static void initialize_apkenv_dl_lock(void)
{
	pthread_mutexattr_t attributes;
	pthread_mutexattr_init(&attributes);
	/*
	 * Constructors are allowed to call dlopen/dlsym. bionic_dlopen invokes
	 * constructors while holding this lock, so the loader lock must be
	 * recursive just like Android's linker lock.
	 */
	pthread_mutexattr_settype(&attributes, PTHREAD_MUTEX_RECURSIVE);
	pthread_mutex_init(&apkenv_dl_lock, &attributes);
	pthread_mutexattr_destroy(&attributes);
}

static void lock_apkenv_dl(void)
{
	pthread_once(&apkenv_dl_lock_once, initialize_apkenv_dl_lock);
	pthread_mutex_lock(&apkenv_dl_lock);
}

static void unlock_apkenv_dl(void)
{
