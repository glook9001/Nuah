#include <stdio.h>
#include <stdarg.h>

#include "libc-stdio.h"

// Bionic standard stream support pre-M Android
// Post-M it's saner and they point to stdin/stdout/stderr symbols instead
const struct bionic___sFILE bionic___sF[3] = {
    {{'s', 't', 'd', 'i', 'n'}},
    {{'s', 't', 'd', 'o', 'u', 't'}},
    {{'s', 't', 'd', 'e', 'r', 'r'}}};

// these only exist in bionic AFAICT

int __srget(FILE *fp)
{
	return fgetc(bionic_file_to_glibc_file(fp));
}

int __swbuf(int c, FILE *fp)
{
	return fputc(c, bionic_file_to_glibc_file(fp));
}

// libstdc++ uses these directly for standard streams, thus we need to wrap em
// and IO_file wraps aren't enough.

int bionic_fflush(FILE *f)
{
	return fflush(bionic_file_to_glibc_file(f));
}

size_t
bionic_fwrite(const void *ptr, size_t size, size_t nmemb, FILE *stream)
{
	return fwrite(ptr, size, nmemb, bionic_file_to_glibc_file(stream));
}

int bionic_putc(int ch, FILE *f)
{
	return putc(ch, bionic_file_to_glibc_file(f));
}

/*
 * glibc and pre-M Android use incompatible FILE layouts. Every API receiving
 * a FILE pointer must translate Android's __sF standard-stream objects before
 * forwarding it to glibc.
 */
void bionic_clearerr(FILE *stream)
{
	clearerr(bionic_file_to_glibc_file(stream));
}

int bionic_fclose(FILE *stream)
{
	return fclose(bionic_file_to_glibc_file(stream));
}

int bionic_feof(FILE *stream)
{
	return feof(bionic_file_to_glibc_file(stream));
}

int bionic_ferror(FILE *stream)
{
	return ferror(bionic_file_to_glibc_file(stream));
}

char *bionic_fgets(char *buffer, int count, FILE *stream)
{
	return fgets(buffer, count, bionic_file_to_glibc_file(stream));
}

int bionic_fileno(FILE *stream)
{
	return fileno(bionic_file_to_glibc_file(stream));
}

int bionic_fprintf(FILE *stream, const char *format, ...)
{
	va_list arguments;
	va_start(arguments, format);
	int result = vfprintf(bionic_file_to_glibc_file(stream), format, arguments);
	va_end(arguments);
	return result;
}

int bionic_fputc(int character, FILE *stream)
{
	return fputc(character, bionic_file_to_glibc_file(stream));
}

int bionic_fputs(const char *string, FILE *stream)
{
	return fputs(string, bionic_file_to_glibc_file(stream));
}

size_t bionic_fread(void *buffer, size_t size, size_t count, FILE *stream)
{
	return fread(buffer, size, count, bionic_file_to_glibc_file(stream));
}

int bionic_fseek(FILE *stream, long offset, int whence)
{
	return fseek(bionic_file_to_glibc_file(stream), offset, whence);
}

int bionic_fseeko(FILE *stream, off_t offset, int whence)
{
	return fseeko(bionic_file_to_glibc_file(stream), offset, whence);
}

long bionic_ftell(FILE *stream)
{
	return ftell(bionic_file_to_glibc_file(stream));
}

off_t bionic_ftello(FILE *stream)
{
	return ftello(bionic_file_to_glibc_file(stream));
}

int bionic_getc(FILE *stream)
{
	return getc(bionic_file_to_glibc_file(stream));
}

int bionic_setvbuf(FILE *stream, char *buffer, int mode, size_t size)
{
	return setvbuf(bionic_file_to_glibc_file(stream), buffer, mode, size);
}

int bionic_ungetc(int character, FILE *stream)
{
	return ungetc(character, bionic_file_to_glibc_file(stream));
}

int bionic_vfprintf(FILE *stream, const char *format, va_list arguments)
{
	return vfprintf(bionic_file_to_glibc_file(stream), format, arguments);
}

#ifdef __GLIBC__

// Wrapping internal glibc VTABLE functions to handle bionic's pre-M crap
// We define __real_IO_file_xsputn in libc.c so linker will link our library,
// it's not used however for anything.

// no such luck with bionic, we need to wrap every single function that gets passed a FILE *

extern size_t __real_IO_file_xsputn(FILE *f, const void *buf, size_t n);

size_t __wrap_IO_file_xsputn(FILE *f, const void *buf, size_t n)
{
	return __real_IO_file_xsputn(bionic_file_to_glibc_file(f), buf, n);
}

#endif
